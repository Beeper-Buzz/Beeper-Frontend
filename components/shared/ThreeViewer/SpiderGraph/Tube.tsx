...
export const Tube = ({ start, end, color, radius = 0.01 }: any) => {
  ...
  const curve = useMemo(() => new THREE.CatmullRomCurve3([new THREE.Vector3(...start), new THREE.Vector3(...end)]), [start, end]);
  ...
  useFrame(({ clock }) => {
    ...
    if (bulbCapRef.current) {
      const tubeEnd = new THREE.Vector3(end[0] * 0.95, end[1] * 0.95, end[2] * 0.95);
