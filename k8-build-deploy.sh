...
# ============================================================
# beeper-frontend: Build, Push, and Deploy to MicroK8s cluster
# Builds natively on Pi (no cross-compilation needed).
# Run from the Beeper-Frontend directory on the dev machine.
# ============================================================

REGISTRY_HOST="${REGISTRY_HOST:-smokey01}"
REGISTRY_PORT="32000"
IMAGE_NAME="beeper-frontend"
TAG="${TAG:-latest}"
...
echo "=== Step 3: Prune build cache on ${REGISTRY_HOST} ==="
ssh "${REGISTRY_HOST}" sudo docker system prune -af

echo "=== Step 4: Apply Kubernetes manifests ==="
ssh "${KUBECTL_NODE}" sudo microk8s kubectl apply -f - < secrets.yml
...
echo "=== Step 5: Rollout restart ==="
ssh "${KUBECTL_NODE}" sudo microk8s kubectl rollout restart deployment/beeper-frontend -n default
ssh "${KUBECTL_NODE}" sudo microk8s kubectl rollout status deployment/beeper-frontend -n default --timeout=180s

echo ""
echo "=== Deploy complete ==="
ssh "${KUBECTL_NODE}" sudo microk8s kubectl get pods -n default -l app=beeper-frontend -o wide
echo ""
echo "URL: https://beeper.instinct.is"
